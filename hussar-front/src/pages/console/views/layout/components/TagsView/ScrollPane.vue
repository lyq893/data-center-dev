...
export default {
  ...
  methods: {
    ...
    moveToTarget(currentTag) {
      ...
      const oldTagsList = document.querySelectorAll('.tags-view-item');
      const tagsList = [];
      ...
      if ($containerWidth > $scrollWrapper.scrollWidth) {
        ...
        const currentIndex = tagsList.findIndex(item => item === currentTag);
        ...
        if (this.afterContainerWidth === $containerWidth) {
          /**
           * this.closeClientWidth不等于0时，说明时关闭按钮
           * this.closeClientWidth等于时，根据 left和容器宽度 跟 afterNextTagOffsetLeft 和 beforePrevTagOffsetLeft值做对比进行计算
           */
