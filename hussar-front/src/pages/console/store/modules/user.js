...
const user = {
  ...
  mutations: {
    ...
    SET_DEPT_ICON: (state, deptIcon) => {
      ...
      caching.session.set(window.appDeptIcon, deptIcon);
      caching.local.set(window.appDeptIcon, deptIcon);
    },
    ...
    Login({ commit }, { userInfo, headers }) {
      return new Promise((resolve, reject) => {
        ...
        hussarAxiosRequestUtils.logInAction(LoginApi.login, userInfo, null, headers).then(response => {
          ...
          // commit('SET_TENANT_ID', response.user_info.tenantId);
    ...
    LoginUserInfo({ commit }, userInfo) {
      return new Promise((resolve, reject) => {
        ...
        hussarAxiosRequestUtils.getAction(LoginApi.getInformation, userInfo).then(response => {
          ...
          commit('SET_GRADEADMIN', response.data.gradeadmin);
          ...
          // 设置部门图标
          commit('SET_DEPT_ICON', response.data.deptIcon);
    ...
    LogOut({
      ...
      return new Promise((resolve, reject) => {
        ...
        hussarAxiosRequestUtils.postAction(LoginApi.logout, '').then(res => {
          ...
          // cas模式下会传递logoutUrl参数，存在此参数时进行页面跳转
